...
describe 'Admin Invoices Index Page' do
  before :each do
    ...
    @ii_3 = InvoiceItem.create!(invoice_id: @i2.id, item_id: @item_2.id, quantity: 90, unit_price: 12, status: 2)

    @discount1 = @m1.bulk_discounts.create!(description: "discount1", threshold: 5, percentage: 5)
  ...
  it 'should display the total revenue the invoice will generate with the calculations' do
    expect(page).to have_content("Gross Revenue: #{@i1.gross_revenue}")
    expect(page).to have_content("Total Discount: -#{@i1.total_discount}")
    expect(page).to have_content("Total Revenue: $#{@i1.total_revenue}")

    expect(page).to_not have_content(@i2.gross_revenue)
